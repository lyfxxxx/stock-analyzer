<template>
  <div class="chart-container" :class="{ 'chart-dark': isDark }" data-testid="dividend-chart">
...
function updateChart() {
  ...
  const sortedData = [...props.yearlyData]
    .filter(d => d.dividendPayoutRatio !== null && d.dividendPayoutRatio !== undefined)
    .sort((a, b) => a.year - b.year)

  const years = sortedData.map(d => d.year.toString())
  const values = sortedData.map(d => {
    const raw = d.dividendPayoutRatio as number
    // Normalize: decimal (<=1) → percentage; percentage number (>1) → keep as-is
  ...
  const option: EChartsCoreOption = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        return `
          <div style="font-weight: 600; margin-bottom: 4px;">${year}年</div>
